...
We need to create a list, which in R is a sort of 'grouping' of variables, that we will pass to Nimble. It must contain all the necessary information for the analysis.
```{r}
## environment for passing to Nimble
forNimble <- list(y=y[1:21],
                N = 21,
                x=x[1:21],
                xstar=x[22])
```
For each member of the list, the left-hand side is the name as Nimble will see it, and the right-hand side is what we are calling the data in R. Notice that we are eliminating the 22nd data point, which is the strange one. We pass the strange $x$ value as `xstar`.

We now create another list, containing settings for Nimble. Almost all of the time (including here) this is unncessary, because Nimble will guess the starting values and use good default settings; this is for demonstration.
```{r}
...
We now 'compile' the model by telling Nimble to combine the data (in `forNimble`), the setup (in `inits`), and the model definition (in the file `regression.bug`).

```{r results = 'hide'}
nimble.model <- nimble::readBUGSmodel(
  model="regression.bug",
  data=forNimble,
  inits=inits)
...
Having compiled the Nimble model, we can now sample from the joint posterior distribution. `Cmcmc` is what we called our compiled model. `conf$addMonitors()` above tells Nimble which variables we are interested in. Nimble will only report back values of these variables. `niter` is the number of Gibbs sampler iterations to run.
```{r results = 'hide'}
...
We are particularly interested in the posterior predictive distribution for y[22], which is contained in column 4 of the MCMC output in `samples`. We can plot this separately:
...
5. In `forNimble`, correct `x=x[1:21]` to `x=x` (because we're going to use all 22 values in `x`) and `y=y[1:21]` to `y=y2`.
6. Remove `ystar` from the `conf$addMonitors()`, and add `y[22]`. 
7. Change your code to refer to `regression2.bug` instead of `regression.bug`.
8. Run all the (newly edited) code, up to and including plotting the posterior densities in `samples`.

What will happen is that Nimble will see that `y[22]` is missing, and will sample from its posterior distribution! In this way, the Bayesian model correctly accounts for incertainty given the missing data. Plot the posterior of `y[22]`. Do you get the same result as we did previously (you should).



```{r child='solution_R/exercise_2/create_data.Rmd', include = solutions, eval = solutions}
```

```{r child='solution_R/exercise_2/nimble_setup.Rmd', include = solutions, eval = solutions}
```

```{r child='solution_R/exercise_2/nimble_analysis.Rmd', include = solutions, eval = solutions}
```
...
```{r child='solution_R/exercise_3/nimble_setup.Rmd', include = solutions, eval = solutions}
```

```{r child='solution_R/exercise_3/nimble_analysis.Rmd', include = solutions, eval = solutions}
```
